<?php
    session_start();
        ...
        die();  
        session_destroy();
        }else if (  $_SESSION['User']['Tipo_User'] ==='Usuario') {
         echo '
            <script>
...
<body style="background-color: #E0FFFF  " >
	<nav class="navbar navbar-light navbar-expand-md shadow-lg navigation-clean-search">
            <img  width="100px" src="./assets/img/logo.png">
             ...
             <label style="width: 60%;  padding-top: 10px; height: 50px;color: white;  background-color: #0a80fe; border-radius: 20px 20px 20px 0px; text-align: center; font-size: 20px; " >Welcome Teacher <strong> <?php echo $_SESSION['User']['Nombre'] ?></strong></label>   
  ...
  <tbody>
     <?php
        ...
        while( $data = mysqli_fetch_array($resultado,   MYSQLI_ASSOC)){
            ...
            if ($data['Tipo_User']!=='Profesor') {
                echo '
                <tr>
                    <th scope="row">'.++$count.'</th>
                    <td>'.$data['Nombre'].' </td>
                    <td>'.$data['Correo'].' </td>
                    <td><a type="button"class="btn btn"  href="Calificaciones.php?IdUsuarios='.$data['IdUsuarios'].'">Ver</a></td>
                    <td><a type="button"class="btn btn-danger"  href="operaciones/borrar.php?IdUsuarios='.$data['IdUsuarios'].'">Eliminar</a></td>
